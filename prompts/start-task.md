...
3. **Task file**: open `tasks/TASK-____*.md` (or create from `_template.md`, owner = me). Select the
   exact runnable `TASK-____#work-unit` from `bin/olympus next <me>`. Confirm goal, repos,
   **boundaries (Allowed paths)**, DoD, and that `start_after` is satisfied. `merge_after` and
   `needs_ack` are later integration gates, not reasons to postpone reversible development.
...
5. **Branch** (each repo touched):
   ...
   ```
6. **Register** (one Olympus commit+push): work unit → `in_progress`; task → `in_progress`; branch;
   status board; letters for anything I'll need from others — **sent now, not when stuck**.
   `[<handle>] task: TASK-____#unit started`.
